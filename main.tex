...
\begin{inparadesc}
	...
	\item[\color{red}Frobenius:] $\mathbf{A} \in \mathbb{R}^{M \times N}$$\|\mathbf{A}\|_F :=\allowbreak \sqrt{\sum_{i=1}^{M} \sum_{j=1}^{N} |\mathbf{A}_{i, j}|^2} =\allowbreak \sqrt{\operatorname{trace}(\mathbf{A}^\top \mathbf{A})} =\allowbreak \sqrt{\sum_{i=1}^{\min\{m, n\}} \sigma_i^2}$
\end{inparadesc}
